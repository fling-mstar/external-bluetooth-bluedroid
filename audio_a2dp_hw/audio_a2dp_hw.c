...
#if A2DP_CAPTURE_FINISH
#define A2DPDEFAULTHZ    0  // 0 for 48K ; 1  for 44.1K
// mstar main audio card
#define CARD_MAD         0
//buffer size for resample
#define RESAMPLER_BUFFER_SIZE                 96000//1024*10*2
//capure main card pcm audio data
static struct pcm* pcm = NULL;
char*  cap_buf = NULL;
// buffer for capture pcm data, before SRC
#if A2DPDEFAULTHZ
static struct resampler_itfe* resampler = NULL;
char* far_buf = NULL;
// buffer hold request pcm data, after SRC
#endif
#endif
...
//support all audio path output through a2dp
//#if A2DP_CAPTURE_FINISH
//capture main card audio data
#if A2DP_CAPTURE_FINISH
    if (pcm) {
        DEBUG("a2dp debug ");
        if (pcm_read(pcm, cap_buf, bytes)>=0) {
            DEBUG("capture maincard audio--->a2dp %d bytes!!!",bytes);
#if A2DPDEFAULTHZ
            size_t in_frames = bytes/(2*2);
            size_t out_frames = bytes/(2*2);
            resampler->resample_from_input(resampler, (int16_t *)cap_buf, &in_frames,(int16_t *)far_buf, &out_frames);
#endif
        } else {
            ERROR("capture a2dp fail!!!");
        }
    } else {
            ERROR("pcm is NULL!!!");
    }
#if A2DPDEFAULTHZ
    sent = skt_write(out->audio_fd, far_buf,  bytes);
#else
    sent = skt_write(out->audio_fd, cap_buf,  bytes);
#endif
#else
...
static int alsa_open(uint32_t card, uint32_t dev, uint32_t flags, struct pcm **pcm, struct pcm_config *config)
{
    ...
    *pcm = pcm_open(card, dev, flags, config);
    if (!*pcm || !pcm_is_ready(*pcm)) {
        ERROR("alsa open fail %s", pcm_get_error(*pcm));
        pcm_close(*pcm);
        *pcm = NULL;
        return -2;
    }
...
//support all audio path output through a2dp
#if A2DP_CAPTURE_FINISH
//for capture main card audio data
    struct pcm_config config_a2dp ;
    config_a2dp.channels = 2;
    config_a2dp.rate = 48000;
    if (0 == alsa_open(CARD_MAD, 1, PCM_IN, &pcm, &config_a2dp)) {
        DEBUG("cap2bluetooth: open capture success!");
#if A2DPDEFAULTHZ
        if (NULL != resampler) {
            release_resampler(resampler);
            resampler = NULL;
        }
        if (0 == create_resampler(config_a2dp.rate, 44100, 2, RESAMPLER_QUALITY_DEFAULT, NULL, &resampler)) {
            DEBUG("create resampler sucess!!");
        } else {
            ERROR("create_resampler fail!!!");
            pcm_close(pcm);
            pcm=NULL;
            ret = -1;
            goto err_open;
        ...
        ERROR("cap2bluetooth: open pcm for capture fail!");
        pcm=NULL;
        ret = -1;
        goto err_open;
    }
...
static int adev_set_master_volume(struct audio_hw_device *dev, float volume)
{
    ...
    // MStar Android Patch Begin
    // inform audioflinger not to apply system-wide mMasterVolume.
    return 0;
    // MStar Android Patch End
}
...
#if A2DP_CAPTURE_FINISH
    //capture a2dp buffer
    cap_buf = (char *)malloc(RESAMPLER_BUFFER_SIZE/2);
    if (NULL == cap_buf) {
        cap_buf = NULL;
        ERROR("cap buf malloc fail, cap2bluetoothA2DP no support!");
        return -ENOMEM;
    }
#if A2DPDEFAULTHZ
    //resample buffer
    if (cap_buf) {
        far_buf = (char *)malloc(RESAMPLER_BUFFER_SIZE/2);
        if (NULL == far_buf) {
            far_buf = NULL;
            ERROR("far buf malloc fail, cap2bluetoothA2DP no support!");
            return -ENOMEM;
        }
    }
#endif
